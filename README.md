# 🦀 Rust Learning Journey

[![Rust CI](https://github.com/0x-sxmeer/rust-learning/actions/workflows/rust.yml/badge.svg)](https://github.com/0x-sxmeer/rust-learning/actions/workflows/rust.yml)

> My structured path from **Python developer** to **professional Rust developer**.
>
> Following [**The Rust Programming Language**](https://doc.rust-lang.org/book/) (2024 Edition) by Steve Klabnik, Carol Nichols, and Chris Krycho, with contributions from the Rust Community.
>
> 📖 [Read the book online](https://doc.rust-lang.org/book/) · 🎓 [Interactive version (Brown Univ.)](https://rust-book.cs.brown.edu) · 📕 [No Starch Press (print)](https://nostarch.com/rust-programming-language-3rd-edition)

---

## 📂 Repository Structure

This repo is a **Cargo workspace**. Each sub-folder is a standalone binary crate with focused exercises mapped 1:1 to the book's sections. Folders are named after chapters so you can read the book and code side-by-side.

```
rust-learning/
├── Cargo.toml                              # Workspace root
├── README.md
├── .gitignore
├── .github/workflows/rust.yml              # CI: check + test + clippy + fmt
│
├── ch02-guessing-game/                     # Chapter 2  — First project
├── ch03-common-concepts/                   # Chapter 3  — Language basics
│   ├── variables-and-mutability/           #   3.1 let, mut, const, shadowing
│   ├── data-types/                         #   3.2 Scalar & compound types
│   ├── functions/                          #   3.3 fn, parameters, return values
│   └── control-flow/                       #   3.5 if/else, loop, while, for
│
├── ch04-ownership/                         # Chapter 4  — THE key Rust concept
│   ├── what-is-ownership/                  #   4.1 Stack vs heap, move, clone, Copy
│   ├── references-and-borrowing/           #   4.2 &T, &mut T, borrowing rules
│   └── the-slice-type/                     #   4.3 &str, &[T], string slices
│
├── ch05-structs/                           # Chapter 5  — Structuring data
│   ├── defining-structs/                   #   5.1 Struct syntax, field init, update
│   ├── example-program/                    #   5.2 Rectangle area calculator
│   └── methods/                            #   5.3 impl blocks, &self, Self
│
├── ch06-enums/                             # Chapter 6  — Enums & pattern matching
│   ├── defining-enums/                     #   6.1 enum, Option<T>, payload variants
│   ├── match-control-flow/                 #   6.2 match arms, exhaustive matching
│   └── if-let/                             #   6.3 if let, let...else shorthand
│
├── ch07-packages-crates-modules/           # Chapter 7  — Module system
│   ├── packages-and-crates/                #   7.1 Binary vs library crates
│   ├── modules-and-scope/                  #   7.2 mod, pub, privacy rules
│   ├── paths/                              #   7.3 Absolute & relative paths
│   ├── use-keyword/                        #   7.4 use, as, re-exporting
│   └── separating-modules/                 #   7.5 Splitting into files
│
├── ch08-collections/                       # Chapter 8  — Standard collections
│   ├── vectors/                            #   8.1 Vec<T>, indexing, iterating
│   ├── strings/                            #   8.2 String vs &str, UTF-8
│   └── hashmaps/                           #   8.3 HashMap<K,V>, entry API
│
├── ch09-error-handling/                    # Chapter 9  — Errors done right
│   ├── panic/                              #   9.1 panic!, unwinding, abort
│   ├── result/                             #   9.2 Result<T,E>, ?, map_err
│   └── when-to-panic/                      #   9.3 Guidelines: panic vs Result
│
├── ch10-generics-traits-lifetimes/         # Chapter 10 — Type system power
│   ├── generics/                           #   10.1 Generic functions, structs, enums
│   ├── traits/                             #   10.2 Defining, implementing, trait bounds
│   └── lifetimes/                          #   10.3 'a annotations, elision rules
│
├── ch11-testing/                           # Chapter 11 — Writing tests
│   ├── writing-tests/                      #   11.1 #[test], assert!, assert_eq!
│   ├── running-tests/                      #   11.2 Filtering, --ignored, --test-threads
│   └── test-organization/                  #   11.3 Unit vs integration tests
│
├── ch12-minigrep/                          # Chapter 12 — I/O project (single crate)
│                                           #   CLI grep clone: args, file I/O, errors
│
├── ch13-closures-iterators/                # Chapter 13 — Functional features
│   ├── closures/                           #   13.1 |x| x+1, Fn/FnMut/FnOnce, capture
│   ├── iterators/                          #   13.2 .iter(), .map(), .filter(), .collect()
│   └── improving-minigrep/                 #   13.3 Refactoring ch12 with iterators
│
├── ch14-cargo-cratesio/                    # Chapter 14 — Cargo mastery
│   └── workspaces/                         #   14.3 Workspace configuration
│
├── ch15-smart-pointers/                    # Chapter 15 — Smart pointers
│   ├── box-heap/                           #   15.1 Box<T>, recursive types
│   ├── deref-trait/                        #   15.2 Deref coercion, *operator
│   ├── drop-trait/                         #   15.3 Drop, std::mem::drop
│   ├── rc-reference-counting/              #   15.4 Rc<T>, Rc::clone, strong_count
│   ├── refcell-interior-mutability/        #   15.5 RefCell<T>, borrow(), borrow_mut()
│   └── reference-cycles/                   #   15.6 Weak<T>, preventing memory leaks
│
├── ch16-concurrency/                       # Chapter 16 — Fearless concurrency
│   ├── threads/                            #   16.1 thread::spawn, JoinHandle, move
│   ├── message-passing/                    #   16.2 mpsc channels, tx/rx
│   ├── shared-state/                       #   16.3 Mutex<T>, Arc<T>
│   └── send-sync/                          #   16.4 Send & Sync marker traits
│
├── ch17-async/                             # Chapter 17 — Async programming (NEW in 2024 ed.)
│   ├── futures-and-syntax/                 #   17.1 async fn, .await, Future trait
│   ├── concurrency-with-async/             #   17.2 join!, select!, task spawning
│   ├── working-with-futures/               #   17.3 Pin, multiple futures
│   ├── streams/                            #   17.4 Stream trait, async iteration
│   └── async-traits/                       #   17.5 async in trait methods
│
├── ch18-oop/                               # Chapter 18 — OOP in Rust
│   ├── characteristics-of-oo/              #   18.1 Encapsulation, pub/private
│   ├── trait-objects/                       #   18.2 dyn Trait, dynamic dispatch
│   └── state-pattern/                      #   18.3 State pattern, typestate
│
├── ch19-patterns/                          # Chapter 19 — Pattern matching mastery
│   ├── all-the-places/                     #   19.1 match, if let, while let, for, let
│   ├── refutability/                       #   19.2 Irrefutable vs refutable patterns
│   └── pattern-syntax/                     #   19.3 Destructuring, guards, @ bindings
│
├── ch20-advanced/                          # Chapter 20 — Advanced features
│   ├── unsafe-rust/                        #   20.1 unsafe blocks, raw pointers, FFI
│   ├── advanced-traits/                    #   20.2 Associated types, default types
│   ├── advanced-types/                     #   20.3 Newtype, type aliases, !
│   ├── advanced-functions/                 #   20.4 fn pointers, returning closures
│   └── macros/                             #   20.5 macro_rules!, proc macros, derive
│
├── ch21-web-server/                        # Chapter 21 — Final project (single crate)
│                                           #   Multithreaded web server with thread pool
│
└── notes/                                  # Personal learning notes
    ├── ownership-cheatsheet.md
    ├── python-vs-rust.md
    └── aha-moments.md
```

---

## 📊 Progress Tracker

### Part I — Getting Started

| # | Chapter | Folder | Exercises | Status |
|---|---------|--------|-----------|--------|
| 2 | [Programming a Guessing Game](https://doc.rust-lang.org/book/ch02-00-guessing-game-tutorial.html) | `ch02-guessing-game/` | 1 project | 📅 Planned |
| 3 | [Common Programming Concepts](https://doc.rust-lang.org/book/ch03-00-common-programming-concepts.html) | `ch03-common-concepts/` | 4 exercises | 🔄 In Progress |

### Part II — Ownership & Structs

| # | Chapter | Folder | Exercises | Status |
|---|---------|--------|-----------|--------|
| 4 | [Understanding Ownership](https://doc.rust-lang.org/book/ch04-00-understanding-ownership.html) | `ch04-ownership/` | 3 exercises | 📅 Planned |
| 5 | [Using Structs](https://doc.rust-lang.org/book/ch05-00-structs.html) | `ch05-structs/` | 3 exercises | 📅 Planned |
| 6 | [Enums and Pattern Matching](https://doc.rust-lang.org/book/ch06-00-enums.html) | `ch06-enums/` | 3 exercises | 📅 Planned |

### Part III — Thinking in Rust

| # | Chapter | Folder | Exercises | Status |
|---|---------|--------|-----------|--------|
| 7 | [Packages, Crates, and Modules](https://doc.rust-lang.org/book/ch07-00-managing-growing-projects-with-packages-crates-and-modules.html) | `ch07-packages-crates-modules/` | 5 exercises | 📅 Planned |
| 8 | [Common Collections](https://doc.rust-lang.org/book/ch08-00-common-collections.html) | `ch08-collections/` | 3 exercises | 📅 Planned |
| 9 | [Error Handling](https://doc.rust-lang.org/book/ch09-00-error-handling.html) | `ch09-error-handling/` | 3 exercises | 📅 Planned |

### Part IV — Generics & Testing

| # | Chapter | Folder | Exercises | Status |
|---|---------|--------|-----------|--------|
| 10 | [Generic Types, Traits, and Lifetimes](https://doc.rust-lang.org/book/ch10-00-generics.html) | `ch10-generics-traits-lifetimes/` | 3 exercises | 📅 Planned |
| 11 | [Writing Automated Tests](https://doc.rust-lang.org/book/ch11-00-testing.html) | `ch11-testing/` | 3 exercises | 📅 Planned |
| 12 | [I/O Project: minigrep](https://doc.rust-lang.org/book/ch12-00-an-io-project.html) | `ch12-minigrep/` | 1 project | 📅 Planned |

### Part V — Advanced Rust

| # | Chapter | Folder | Exercises | Status |
|---|---------|--------|-----------|--------|
| 13 | [Closures and Iterators](https://doc.rust-lang.org/book/ch13-00-functional-features.html) | `ch13-closures-iterators/` | 3 exercises | 📅 Planned |
| 14 | [More About Cargo](https://doc.rust-lang.org/book/ch14-00-more-about-cargo.html) | `ch14-cargo-cratesio/` | 1 exercise | 📅 Planned |
| 15 | [Smart Pointers](https://doc.rust-lang.org/book/ch15-00-smart-pointers.html) | `ch15-smart-pointers/` | 6 exercises | 📅 Planned |
| 16 | [Fearless Concurrency](https://doc.rust-lang.org/book/ch16-00-concurrency.html) | `ch16-concurrency/` | 4 exercises | 📅 Planned |
| 17 | [Async and Await](https://doc.rust-lang.org/book/ch17-00-async-await.html) | `ch17-async/` | 5 exercises | 📅 Planned |
| 18 | [OOP Features of Rust](https://doc.rust-lang.org/book/ch18-00-oop.html) | `ch18-oop/` | 3 exercises | 📅 Planned |
| 19 | [Patterns and Matching](https://doc.rust-lang.org/book/ch19-00-patterns.html) | `ch19-patterns/` | 3 exercises | 📅 Planned |
| 20 | [Advanced Features](https://doc.rust-lang.org/book/ch20-00-advanced-features.html) | `ch20-advanced/` | 5 exercises | 📅 Planned |

### Final Project

| # | Chapter | Folder | Type | Status |
|---|---------|--------|------|--------|
| 21 | [Building a Multithreaded Web Server](https://doc.rust-lang.org/book/ch21-00-final-project-a-web-server.html) | `ch21-web-server/` | Capstone project | 📅 Planned |

**Total: 21 chapters · 62 exercises · 3 projects**

---

## 📝 Exercise Breakdown by Chapter

<details>
<summary><strong>Ch 2 — Programming a Guessing Game</strong></summary>

Single project combining: `use std::io`, `rand` crate, `loop`, `match`, type parsing.

```bash
cargo run -p guessing-game
```
</details>

<details>
<summary><strong>Ch 3 — Common Programming Concepts</strong> (4 exercises)</summary>

| Exercise | Section | Concepts Practiced |
|----------|---------|-------------------|
| `variables-and-mutability` | [§3.1](https://doc.rust-lang.org/book/ch03-01-variables-and-mutability.html) | `let`, `mut`, `const`, shadowing, type re-binding |
| `data-types` | [§3.2](https://doc.rust-lang.org/book/ch03-02-data-types.html) | `i32`/`u64`/`f64`, `bool`, `char`, tuples, arrays, integer overflow |
| `functions` | [§3.3](https://doc.rust-lang.org/book/ch03-03-how-functions-work.html) | Function parameters, return values, expressions vs statements, `-> Type` |
| `control-flow` | [§3.5](https://doc.rust-lang.org/book/ch03-05-control-flow.html) | `if`/`else`, `if` in `let`, `loop`, `while`, `for`, labels, `break` with value |

</details>

<details>
<summary><strong>Ch 4 — Understanding Ownership</strong> (3 exercises) ⭐ Most Important Chapter</summary>

| Exercise | Section | Concepts Practiced |
|----------|---------|-------------------|
| `what-is-ownership` | [§4.1](https://doc.rust-lang.org/book/ch04-01-what-is-ownership.html) | Stack vs heap, ownership rules, variable scope, `String` type, `move`, `clone`, `Copy` trait |
| `references-and-borrowing` | [§4.2](https://doc.rust-lang.org/book/ch04-02-references-and-borrowing.html) | `&T` (shared ref), `&mut T` (mutable ref), "one mutable XOR many shared" rule, dangling references |
| `the-slice-type` | [§4.3](https://doc.rust-lang.org/book/ch04-03-slices.html) | `&str` string slices, `&[i32]` array slices, range syntax `[0..5]`, why slices prevent bugs |

> 💡 **Python dev note**: This chapter has no Python equivalent. Python uses garbage collection — Rust uses ownership. Spend extra time here.

</details>

<details>
<summary><strong>Ch 5 — Using Structs</strong> (3 exercises)</summary>

| Exercise | Section | Concepts Practiced |
|----------|---------|-------------------|
| `defining-structs` | [§5.1](https://doc.rust-lang.org/book/ch05-01-defining-structs.html) | Named fields, tuple structs, unit structs, field init shorthand, struct update syntax `..` |
| `example-program` | [§5.2](https://doc.rust-lang.org/book/ch05-02-example-structs.html) | `Rectangle` area calculator, `#[derive(Debug)]`, `dbg!` macro, `{:?}` formatting |
| `methods` | [§5.3](https://doc.rust-lang.org/book/ch05-03-method-syntax.html) | `impl` blocks, `&self`, `&mut self`, `Self`, associated functions (like Python `@staticmethod`) |

</details>

<details>
<summary><strong>Ch 6 — Enums and Pattern Matching</strong> (3 exercises)</summary>

| Exercise | Section | Concepts Practiced |
|----------|---------|-------------------|
| `defining-enums` | [§6.1](https://doc.rust-lang.org/book/ch06-01-defining-an-enum.html) | `enum` with data payloads, `Option<T>` (`Some`/`None`), why Rust has no null |
| `match-control-flow` | [§6.2](https://doc.rust-lang.org/book/ch06-02-match.html) | `match` arms, exhaustive matching, `_` catch-all, binding values |
| `if-let` | [§6.3](https://doc.rust-lang.org/book/ch06-03-if-let.html) | `if let Some(x) = ...`, `let...else`, concise control flow |

</details>

<details>
<summary><strong>Ch 7 — Packages, Crates, and Modules</strong> (5 exercises)</summary>

| Exercise | Section | Concepts Practiced |
|----------|---------|-------------------|
| `packages-and-crates` | [§7.1](https://doc.rust-lang.org/book/ch07-01-packages-and-crates.html) | Crate root, binary vs library crates, package rules |
| `modules-and-scope` | [§7.2](https://doc.rust-lang.org/book/ch07-02-defining-modules-to-control-scope-and-privacy.html) | `mod`, module tree, privacy boundary, `pub` |
| `paths` | [§7.3](https://doc.rust-lang.org/book/ch07-03-paths-for-referring-to-an-item-in-the-module-tree.html) | `crate::`, `self::`, `super::`, absolute vs relative |
| `use-keyword` | [§7.4](https://doc.rust-lang.org/book/ch07-04-bringing-paths-into-scope-with-the-use-keyword.html) | `use`, `as` aliases, `pub use` re-exporting, nested paths |
| `separating-modules` | [§7.5](https://doc.rust-lang.org/book/ch07-05-separating-modules-into-different-files.html) | Multi-file modules, `mod.rs` vs `filename.rs` |

</details>

<details>
<summary><strong>Ch 8 — Common Collections</strong> (3 exercises)</summary>

| Exercise | Section | Concepts Practiced |
|----------|---------|-------------------|
| `vectors` | [§8.1](https://doc.rust-lang.org/book/ch08-01-vectors.html) | `Vec<T>`, `vec![]`, push, indexing vs `.get()`, iterating, enum trick for mixed types |
| `strings` | [§8.2](https://doc.rust-lang.org/book/ch08-02-strings.html) | `String` vs `&str`, UTF-8, concatenation, `format!`, why no indexing |
| `hashmaps` | [§8.3](https://doc.rust-lang.org/book/ch08-03-hash-maps.html) | `HashMap<K,V>`, `.insert()`, `.entry().or_insert()`, ownership of keys |

</details>

<details>
<summary><strong>Ch 9 — Error Handling</strong> (3 exercises)</summary>

| Exercise | Section | Concepts Practiced |
|----------|---------|-------------------|
| `panic` | [§9.1](https://doc.rust-lang.org/book/ch09-01-unrecoverable-errors-with-panic.html) | `panic!`, `RUST_BACKTRACE=1`, unwinding vs aborting |
| `result` | [§9.2](https://doc.rust-lang.org/book/ch09-02-recoverable-errors-with-result.html) | `Result<T,E>`, `match`, `unwrap_or_else`, `?` operator, error propagation |
| `when-to-panic` | [§9.3](https://doc.rust-lang.org/book/ch09-03-to-panic-or-not-to-panic.html) | Guidelines, custom types for validation, prototype vs production |

</details>

<details>
<summary><strong>Ch 10 — Generic Types, Traits, and Lifetimes</strong> (3 exercises)</summary>

| Exercise | Section | Concepts Practiced |
|----------|---------|-------------------|
| `generics` | [§10.1](https://doc.rust-lang.org/book/ch10-01-syntax.html) | `fn largest<T>`, generic structs/enums, monomorphization (zero-cost!) |
| `traits` | [§10.2](https://doc.rust-lang.org/book/ch10-02-traits.html) | `trait Summary`, `impl Trait for Type`, default impls, trait bounds, `where` clauses |
| `lifetimes` | [§10.3](https://doc.rust-lang.org/book/ch10-03-lifetime-syntax.html) | `'a` annotations, lifetime elision rules, `'static`, struct lifetimes |

</details>

<details>
<summary><strong>Ch 11 — Writing Automated Tests</strong> (3 exercises)</summary>

| Exercise | Section | Concepts Practiced |
|----------|---------|-------------------|
| `writing-tests` | [§11.1](https://doc.rust-lang.org/book/ch11-01-writing-tests.html) | `#[test]`, `assert!`, `assert_eq!`, `assert_ne!`, custom messages, `#[should_panic]`, `Result` in tests |
| `running-tests` | [§11.2](https://doc.rust-lang.org/book/ch11-02-running-tests.html) | `--test-threads`, `--show-output`, filtering by name, `#[ignore]` |
| `test-organization` | [§11.3](https://doc.rust-lang.org/book/ch11-03-test-organization.html) | Unit tests (`#[cfg(test)]`), integration tests (`tests/`), testing private functions |

</details>

<details>
<summary><strong>Ch 12 — I/O Project: minigrep</strong> (single project)</summary>

A complete CLI program that searches for a string in a file — like a simplified `grep`.

Concepts combined: Command-line args, file I/O, `Result` error handling, `process::exit`, refactoring into `lib.rs`, environment variables, `stderr` vs `stdout`, TDD.

```bash
cargo run -p minigrep -- searchstring poem.txt
```
</details>

<details>
<summary><strong>Ch 13–21: Functional, Advanced, and Final Project</strong></summary>

See the full structure in the folder tree above. These chapters build on everything before:

- **Ch 13**: Closures, iterators, and refactoring minigrep with functional style
- **Ch 14**: Cargo release profiles, publishing crates, workspaces
- **Ch 15**: Smart pointers (`Box`, `Rc`, `RefCell`), `Deref`/`Drop` traits, reference cycles
- **Ch 16**: Threads, channels (`mpsc`), shared state with `Arc<Mutex<T>>`
- **Ch 17**: Async/await, `Future`, streams *(new in 2024 edition)*
- **Ch 18**: OOP patterns, trait objects (`dyn Trait`), state pattern
- **Ch 19**: Advanced pattern matching, destructuring, guards
- **Ch 20**: Unsafe Rust, advanced traits, macros
- **Ch 21**: **Final project** — building a multithreaded web server from scratch

</details>

---

## 🚀 How to Use This Repo

```bash
# Clone
git clone https://github.com/0x-sxmeer/rust-learning.git
cd rust-learning

# Run a specific exercise
cargo run -p variables-and-mutability
cargo run -p guessing-game

# Run all tests
cargo test --workspace

# Lint everything
cargo clippy --workspace -- -D warnings

# Format everything
cargo fmt --all
```

### Adding a New Exercise

```bash
# Example: starting Chapter 4
cargo new ch04-ownership/what-is-ownership

# Cargo auto-adds it to the workspace. Write code, then:
git add .
git commit -m "feat(ch04): add ownership basics — move, clone, Copy"
git push
```

### Commit Message Convention

```
feat(ch04): add ownership basics — move semantics and scope
fix(ch03):  correct type mismatch in data-types exercise
docs:       update README progress for Chapter 5
refactor:   extract helper function in minigrep
test(ch11): add integration tests for search functionality
```

---

## 🐍 Python → Rust Quick Reference

| Concept | Python | Rust |
|---------|--------|------|
| Package manager | `pip` | `cargo` |
| Project config | `pyproject.toml` | `Cargo.toml` |
| Locked deps | `requirements.txt` | `Cargo.lock` |
| Virtual env | `venv` / `virtualenv` | Not needed — Cargo isolates automatically |
| Run code | `python main.py` | `cargo run` |
| Add dependency | `pip install requests` | `cargo add reqwest` |
| Test | `pytest` | `cargo test` |
| Format | `black` / `ruff format` | `cargo fmt` |
| Lint | `pylint` / `ruff check` | `cargo clippy` |
| Type check | `mypy` | The compiler itself |
| Package registry | PyPI | [crates.io](https://crates.io) |
| REPL | `python` | Not built-in (try [evcxr](https://github.com/evcxr/evcxr)) |
| Classes | `class Foo:` | `struct Foo` + `impl Foo` |
| Inheritance | `class B(A):` | Traits (composition > inheritance) |
| Exceptions | `try`/`except` | `Result<T,E>` + `?` operator |
| None | `None` | `Option<T>` (`Some(v)` / `None`) |
| GC | Garbage collector | Ownership + borrowing (zero-cost) |
| Concurrency | `threading` / `asyncio` | `std::thread` / `async`/`.await` |
| Docstrings | `"""..."""` | `///` doc comments |

---

## 📚 Resources

| Resource | Type | Link |
|----------|------|------|
| **The Rust Programming Language** | 📖 Official Book | [doc.rust-lang.org/book](https://doc.rust-lang.org/book/) |
| **Interactive Rust Book** | 🎓 With Quizzes | [rust-book.cs.brown.edu](https://rust-book.cs.brown.edu) |
| **Rust by Example** | 💻 Code-First | [doc.rust-lang.org/rust-by-example](https://doc.rust-lang.org/rust-by-example/) |
| **Rustlings** | 🏋️ Exercises | [github.com/rust-lang/rustlings](https://github.com/rust-lang/rustlings) |
| **Rust Playground** | 🧪 Try Online | [play.rust-lang.org](https://play.rust-lang.org/) |
| **Rust Standard Library** | 📘 API Docs | [doc.rust-lang.org/std](https://doc.rust-lang.org/std/) |
| **Crates.io** | 📦 Package Registry | [crates.io](https://crates.io) |
| **This Week in Rust** | 📰 Newsletter | [this-week-in-rust.org](https://this-week-in-rust.org/) |

---

## ⚙️ CI/CD

Every push to `main` automatically runs:

| Check | Command | Purpose |
|-------|---------|---------|
| ✅ Compile | `cargo check --workspace` | Does it build? |
| ✅ Test | `cargo test --workspace` | Do tests pass? |
| ✅ Lint | `cargo clippy --workspace` | Idiomatic code? |
| ✅ Format | `cargo fmt --all -- --check` | Consistent style? |

---

## 📄 Book Edition Note

This repo follows the **2024 Edition** (Rust 1.90+) of The Rust Programming Language, which includes:
- Chapter 17 on **async/await** (not in the 2nd print edition)
- `edition = "2024"` in all `Cargo.toml` files
- Updated chapter numbering (21 chapters vs 20 in the No Starch Press 2nd edition)

If you're reading the [No Starch Press paperback](https://nostarch.com/rust-programming-language-3rd-edition), chapters 1–16 map directly. Chapters 17+ differ — use the [free online version](https://doc.rust-lang.org/book/) as the primary reference.

---

<p align="center">
  <sub>Started: September 2026 · Rust Edition 2024 · Built with 🦀 and determination</sub>
  <br>
  <sub>By <a href="https://github.com/0x-sxmeer">@0x-sxmeer</a> — Python dev becoming a Rustacean</sub>
</p>
